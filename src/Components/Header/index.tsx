...
const Header: React.FC<IPropsModal> = () => {
  ...
  const addDessert = async (dessert: string) => {
    ...
    await api.post("desserts/dessert", body, { headers: header }).then((response) => {
      if (response.status === 200) {
  ...
  const addMovie = async (movie: string) => {
    ...
    await api.post("movies/movie", body, { headers: header }).then((response) => {
      if (response.status === 200) {
        setOpenMovie(false)
      } else {
        alert("deu ruiim");
  ...
  const addDrink = async (drink: string) => {
    ...
    await api.post("drinks/drink", body, { headers: header }).then((response) => {
      if (response.status === 200) {
        setOpenDrink(false)
      } else {
        alert("deu ruiim");
